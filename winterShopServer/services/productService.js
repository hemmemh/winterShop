  ...
  async getAll(query) {
    try {
      ...
      if (checkedBrands !== 'false') {
        console.log('++++');
        responce = [
          ...responce.filter((e) => e.brand._id.toString() == checkedBrands),
        ];
      }
      ...
      if (sizes.length !== 0) {
        responce = [
          ...responce.filter((e) =>
            JSON.parse(e.sizes).find((el) => sizes.includes(el))
          ),
        ];
      }
      ...
      if (colors.length !== 0) {
        ...
        responce = [
          ...responce.filter((e) =>
            Object.values(JSON.parse(e.colors)).find((el) =>
              colorsTwo.includes(el)
            )
          ),
        ];
      }
  ...
  async createMany() {
    try {
      ...
      for (const product of products) {
        const { colors, description, images, name, price, sizes, type } =
          product;
        const date = Date.now();
