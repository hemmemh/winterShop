  ...
  <router-link :to="`/product/${product._id}`">
    <div class="product-catalog__item product-item">
      <div class="product-item__imageCover">
        <div
          class="product-item__star _icon-star-winter"
          :class="{ active: inLovesData }"
          @click.stop.prevent="setLoves"
        ></div>
        <div class="product-item__image">
          <img
            :src="`${API_URL}/${product.name}/${JSON.parse(product.images)[0]}`"
            alt=""
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async setLoves() {
      this.inLovesData = await this.addLoves({
        id: this.product._id,
        lovesId: this.$store.state.user.user.loves,
        inLovesData: this.inLovesData,
      });
    },
  },
};
