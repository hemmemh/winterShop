<template lang="">
  <div class="Basket__item item-basket">
    <div class="item-basket__remove" @click="removeItem"></div>
    <div class="item-basket__image">
      <div class="item-basket__imageCover">
        <div class="item-basket__imageAbsolute">
          <img
            :src="`${API_URL}/${item.product.name}/${JSON.parse(item.product.images)[0]}`"
            alt=""
          />
        </div>
      </div>
    ...
    <div class="item-basket__count">
      <div class="item-basket__change _minus" @click="minus"></div>
      <div class="item-basket__countValue">
        <MyNumberVue
          ...
          :min="1"
          :max="30"
          :model-value="countItem"
          @update:model-value="setCountItem"
        />
      </div>

      <div class="item-basket__change _plus" @click="plus"></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setCountItem(e) {
      this.setTotalSum(
        this.totalSum -
          this.countItem * this.item.product.price +
          e * this.item.product.price
      );
    ...
    plus() {
      if (this.countItem < 30) {
        this.countItem += 1;
        changeAmount({ id: this.item._id, count: this.countItem }).then(() => {
          this.setTotalSum(
            this.totalSum -
              (this.countItem - 1) * this.item.product.price +
              this.countItem * this.item.product.price
          );
        });
      }
    ...
    removeItem() {
      removeItemFromBasket({
        ...
        basketId: store.state.user.user.basket,
      }).then((data) => {
        this.$emit('removeItem', data);
        this.setTotalSum(
          this.totalSum - this.item.product.price * this.countItem
        );
      });
    },
  },
