<template lang="">
  <div
    class="loginModal__side _right"
    :class="{ hide: loginNavBarChange && login }"
  >
    ...
    <div class="loginModal__inputs">
      <div class="loginModal__inputCover">
        ...
        <MyInputVue v-model="name" input-class="login" />
      </div>
      <div class="loginModal__inputCover">
        <div class="loginModal__inputName">Номер телефона или email</div>
        <MyInputVue
          v-model="emailReg"
          :input-class="emailRegTest ? 'login active' : 'login'"
          @update:model-value="setEmailReg"
        />
      </div>
      <div class="loginModal__inputCover">
        <div class="loginModal__inputName">Пароль</div>
        <MyInputVue v-model="passwordReg" input-class="login" />
      </div>
    </div>

    <MyButtonVue class-name="login _reg" @click="onRegistration"
      >Зарегистрироваться</MyButtonVue
    >
    <div v-if="errorRegEl" class="loginModal__error">{{ errorRegText }}</div>
    <div
      class="loginModal__change"
      :class="{ active: loginNavBarChange }"
      @click="setLogin(true)"
    >
      Войти
    </div>
...
export default {
  ...
  methods: {
    ...
    async onRegistration() {
      ...
      let { name, passwordReg, emailReg, errorRegText, errorRegEl } =
        await this.onRegistrationAction({
          name: this.name,
          passwordReg: this.passwordReg,
          emailReg: this.emailReg,
          errorRegText: this.errorRegText,
          errorRegEl: this.errorRegEl,
        });
      this.name = name;
