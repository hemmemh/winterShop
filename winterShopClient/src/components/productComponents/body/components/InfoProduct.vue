<template lang="">
  <div class="Product__info info-product">
    <div class="info-product__top">
      ...
      <div class="info-product__ratingCount" @click="toRatingBlock">
        {{ product.ratings.length }} отзыв(а)
      </div>
    </div>
...
export default {
  ...
  computed: {
    productRating() {
      return (
        Math.round(
          this.product.ratings.reduce((a, b) => {
            return a + b.rate;
          }, 0) / this.product.ratings.length
        ) || 0
      );
    },
  },
  methods: {
    toRatingBlock() {
      ...
      window.scrollTo({
        behavior: 'smooth',
        top: rating.getBoundingClientRect().top - navBar.offsetHeight,
      });
    },
  },
};
